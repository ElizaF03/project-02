...
private string $date;
    public function __construct(int $id, $userId, string $firstName, string $lastName, string $address, string $phone, string $totalPrice, string $date){
        $this->id = $id;
        $this->userId = $userId;
    ...
    public static function getOrder($userId):Order{
        ...
        $result= $stmt->fetch();
        $obj= new self($result['id'], $result['userId'], $result['first_name'], $result['last_name'], $result['address'], $result['phone'], $result['total_price'], $result['date']);
        return $obj;
    }
}